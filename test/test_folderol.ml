...
run_goal
  "(ALL x. ALL y. P(x) --> Q(y))                       <-> ((EXISTS x. P(x)) \
   --> (ALL y. Q(y)))"
;;

(*Principia Mathematica *11.55  *)
run_goal
  "(EXISTS x. EXISTS y. P(x) & Q(x,y))                       <-> (EXISTS x. \
   P(x) & (EXISTS y. Q(x,y)))"
;;

(*Principia Mathematica *11.61  *)
run_goal
  "(EXISTS y. ALL x. P(x) --> Q(x,y))                         --> (ALL x. P(x) \
   --> (EXISTS y. Q(x,y)))"
;;
...
steps 40;;
print_endline "Don't expect to have proved this one...\n";;

(*the simpler goal is provable...*)
run_goalseq ([ "ALL x. P(x)-->Q(x)" ], [ "ALL x. ~Q(x) --> ~P(x)" ]);;
print_endline "Pelletier's Problems\n";;
...
run_goalseq
  ( [ "(EXISTS x. P(x)) & (EXISTS x. Q(x))" ],
    [
      "(ALL x. P(x)-->R(x)) & (ALL x. Q(x)-->S(x)) <->              (ALL x. \
       ALL y. P(x) & Q(y) --> R(x) & S(y))";
    ] )
;;

print_endline "Problem 30\n";;
...
(*33*)
run_goal
  "(ALL x. P(a) & (P(x)-->P(b))-->P(c))  <->       (ALL x. (~P(a) | P(x) | \
   P(c)) & (~P(a) | ~P(b) | P(c)))"
;;

(*34  AMENDED.  Not proved!!!*)
read_goal
  "((EXISTS x. ALL y. P(x) <-> P(y))  <->               ((EXISTS x. Q(x)) <-> \
   (ALL y. Q(y))))     <->              ((EXISTS x. ALL y. Q(x) <-> Q(y))  \
   <->               ((EXISTS x. P(x)) <-> (ALL y. P(y))))"
;;

steps 40;;
print_endline "Don't expect to have proved this one...\n";;
print_endline "Problem 35\n";;
...
(*38  Not proved!!!*)
read_goal
  "(ALL x. P(a1) & (P(x)--> (EXISTS y. P(y)&R(x,y))) \
   -->                        (EXISTS z. EXISTS w.P(z) & R(x,w) & R(w,z)))  \
   <->       (ALL x. (~P(a1) | P(x) | (EXISTS z. EXISTS w.P(z) & R(x,w) & \
   R(w,z))) &              (~P(a1) | ~(EXISTS y. P(y)&R(x,y)) \
   |                   (EXISTS z. EXISTS w.P(z) & R(x,w) & R(w,z))))"
;;
...
print_endline "Problem 40\n";;

(*40.  AMENDED*)
run_goal
  "(EXISTS y. ALL x. F(y,x) <-> ~F(x,x))  -->               ~ (ALL x. EXISTS \
   y. ALL z. F(z,y) <-> ~ F(z,x))"
;;
...
read_goalseq
  [
    "ALL x. F(x) -->                ((EXISTS y. G(y) & H(x,y)) & (EXISTS y. \
     G(y) & ~ H(x,y)))";
...
run_goalseq
  ( [
      "ALL x. F(x) & (ALL y. G(y) & H(x,y) --> J(x,y))                        \
       --> (ALL y. G(y) & H(x,y) --> K(y))";
      "~ (EXISTS y. L(y) & K(y))";
      "EXISTS x. F(x) & (ALL y. H(x,y) --> L(y))                     & (ALL y. \
       G(y) & H(x,y) --> J(x,y))";
    ],
...
(*takes 50 seconds!!!*)
read_goal "EXISTS x. P(x) --> P(a) & P(b)";;
step ();;
step ();;
step ();;
step ();;
